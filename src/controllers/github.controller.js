...
import { SUCCESSFUL_CONVERSION_STATUSES } from '../utils/helpers.js';



const getStoredGithubToken = (user) => {
  ...
  try {
    ...
    logger.error('Failed to decrypt stored GitHub access token:', error);
    return null;
  }
};
...
export const githubCallback = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const result = await AuthService.handleGithubAuth({
    ...
    // Set JWT as HttpOnly cookie (NOT in URL)
    ...
    // Redirect to frontend callback (NO token in URL, cookie is set automatically)
...
export const listUserRepos = asyncHandler(async (req, res) => {
  ...
  const storedToken = getStoredGithubToken(user);
  if (!user || !storedToken) {
...
export const cloneRepository = asyncHandler(async (req, res) => {
  ...
  const storedToken = getStoredGithubToken(user);
  const hasAuth = storedToken || github_token;
...
export const pushConvertedProject = asyncHandler(async (req, res) => {
  ...
  if (!accessToken) {
    const user = await UserModel.findById(userId);
    accessToken = getStoredGithubToken(user);
  }
  ...
  if (!SUCCESSFUL_CONVERSION_STATUSES.has(conversionJob.status)) {
    return res.status(400).json({
      success: false,
      error: {
...
export const getGithubStatus = asyncHandler(async (req, res) => {
  ...
  const user = await UserModel.findById(userId);
  const hasOAuthToken = !!getStoredGithubToken(user);
