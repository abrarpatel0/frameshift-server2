...
import { toPositiveInt, SUCCESSFUL_CONVERSION_STATUSES } from '../utils/helpers.js';
...
const normalizeAccuracyGoal = (value) => {
...
export const startConversion = asyncHandler(async (req, res) => {
  ...
  const accuracyGoal = normalizeAccuracyGoal(rawAccuracyGoal);
  let customApiConfig = null;

  if (accuracyGoal === 'high') {
    if (!use_ai) {
      return res.status(400).json({
        success: false,
        error: {
          message: 'High Accuracy mode requires AI enhancement to be enabled.'
        }
      });
    }
    ...
    if (conversionMode !== 'custom') {
      return res.status(400).json({
        success: false,
        error: {
          message: 'High Accuracy mode requires "Use My Own API" with a validated provider key.'
        }
      });
    }
  }
...
export const downloadConversion = asyncHandler(async (req, res) => {
  ...
  if (!SUCCESSFUL_CONVERSION_STATUSES.has(job.status)) {
    return res.status(400).json({
      success: false,
      error: {
        message: 'Conversion has not passed validation yet'
      }
    });
  }
...
export const cancelConversion = asyncHandler(async (req, res) => {
  ...
  if (SUCCESSFUL_CONVERSION_STATUSES.has(job.status) || job.status === 'failed') {
    return res.status(400).json({
      success: false,
      error: {
